...
static void esp_shmem_hw_queue_write(void)
{
	...
	for (;;) {
		...
		hw_q->write = ++w;
		changed = true;
		ESP_LOGD(TAG, "%s: write_queue->write = %"PRId32, __func__, w);
...
static bool esp_shmem_hw_queue_read(void)
{
	...
	while (r != w) {
		...
		if (addr < ESP_IPC_CLIENTS_MAX && client[addr].rx) {
			...
			ESP_LOGE(TAG, "got IPC for an unknown address %"PRId32, addr);
		}

		hw_q->read = ++r;
		ESP_LOGD(TAG, "%s: read_queue->read = %"PRId32" write = %"PRId32, __func__, r, w);
	}
	return true;
}
